    ...
    if (event.type === 'upsert') {
      set((state) => {
        ...
        if (event.request.studentId === state.studentId) {
          // Only update session expiry if request is active and we don't already have one set
          if ((event.request.status === 'pending' || event.request.status === 'assigned') && !nextSessionExpiresAt) {
            nextSessionExpiresAt = event.request.expiresAt;
          } else if (event.request.status === 'resolved') {
            nextSessionExpiresAt = null;
          }
          // Note: For expired requests, we keep the original expiresAt as session expiry
